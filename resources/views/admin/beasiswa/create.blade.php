...
document.addEventListener('DOMContentLoaded', function () {
        ...
        if (data.options && Array.isArray(data.options) && data.options.length > 0) {
            ...
            data.options.forEach(option => {
                ...
                valueInput.addEventListener('input', () => {
                    // Update data when option value changes
                    updateFormFieldData();
                });
                labelInput.addEventListener('input', () => {
                    // Update data when option label changes
                    updateFormFieldData();
                });
                removeBtn.addEventListener('click', () => {
                    optionDiv.remove();
    ...
    function moveFormField(fromIndex, toIndex) {
        if (toIndex < 0 || toIndex >= formFieldsData.length) return;
        const item = formFieldsData.splice(fromIndex, 1)[0];
        formFieldsData.splice(toIndex, 0, item);
        updateFormFieldsDisplay();
    }
    ...
    function moveDocumentItem(fromIndex, toIndex) {
    ...
    // Helper function to update all field data
    function updateFormFieldData() {
        // This function can be called to trigger update of all fields
        formFieldsData.forEach((field, index) => {
            ...
            if (fieldElement) {
                ...
                if (nameInput && nameInput.value && nameInput.value !== field.name) {
                    if (field.key) {
                        removeFromUsedKeys(field.key, false);
                    }
                    field.key = generateUniqueFieldKey(nameInput.value);
    ...
    // Helper function to update all document data
    function updateDocumentData() {
        // This function can be called to trigger update of all documents
        documentsData.forEach((doc, index) => {
            const docElement = documentsContainer.querySelector(`[data-index="${index}"]`);
            if (docElement) {
                const nameInput = docElement.querySelector('.document-name');
                const keyDisplay = docElement.querySelector('.document-key-display');
                
                if (nameInput && nameInput.value && nameInput.value !== doc.name) {
                    if (doc.key) {
                        removeFromUsedKeys(doc.key, true);
                    }
                    doc.key = generateUniqueDocumentKey(nameInput.value);
                    doc.name = nameInput.value;
                    if (keyDisplay) {
                        keyDisplay.textContent = doc.key;
                    }
                }
            }
        });
