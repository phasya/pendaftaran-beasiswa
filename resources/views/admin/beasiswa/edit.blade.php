...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (existingFormFields && Array.isArray(existingFormFields) && existingFormFields.length > 0) {
        ...
        // Add existing keys to tracking set
    ...
    if (existingDocuments && Array.isArray(existingDocuments) && existingDocuments.length > 0) {
        ...
        documentsData.forEach(doc => {
            if (doc.key) usedDocumentKeys.add(doc.key);
        });
    }
    ...
    // Form Fields Management (same as create.blade.php)
    ...
    function setupFormFieldEvents(fieldElement, index) {
        ...
        function updateFormFieldData() {
            if (formFieldsData[index]) {
                // Generate key if name changed and no existing key
    ...
    // Documents Management (same as create.blade.php)
    ...
    function loadDefaultDocuments() {
        if (confirm('Ini akan mengganti semua dokumen yang sudah ada. Lanjutkan?')) {
            documentsData = [
                { name: 'Transkrip Nilai', key: 'file_transkrip_nilai', icon: 'fas fa-file-pdf', color: 'red', formats: ['pdf'], max_size: 5, description: 'Transkrip nilai terbaru', required: true },
